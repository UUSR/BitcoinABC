...
namespace BitcoinABC
{
    public class Secp256K1Manager
    {
        ...
        private static Context Secp256K1ContextCreate(Options flags)
        {
            ...
            {
                var illegalCallback = context2.IllegalCallback;
        ...
        {
            if (((int)sig[skip] & 128) > 0 || (int)sig[skip] == 0 && ((int)sig[skip + 1] & 128) <= 0 || ((int)sig[skip + 32] & 128) > 0)
                return false;
            if ((int)sig[skip + 32] == 0)
        ...
        {
            if (((int)sig[31] & 128) > 0 || (int)sig[31] == 0 && ((int)sig[30] & 128) <= 0 || ((int)sig[63] & 128) > 0)
                return false;
            if ((int)sig[63] == 0)
        ...
        {
            var outputlen = compressed ? 33 : 65;
            ...
            if (outputlen == output.Length)
                return output;
            var numArray = new byte[outputlen];
